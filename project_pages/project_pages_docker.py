...
class ProjectPages_Docker(DockWidget):
    ...
    def Connections(self):
        ...
        # Dialog Mirror
        self.dialog.mf_source.currentTextChanged.connect(self.MirrorFix_Source)
        self.dialog.mf_run.clicked.connect(self.MirrorFix_Run)
        # Notices
    ...
    def Settings(self):
        ...
        # Mirror Fix
        mf_source = str( Krita.instance().readSetting("Project Pages", "mf_source", "") )
        if mf_source == "":
            Krita.instance().writeSetting("Project Pages", "mf_source", "LEFT")
        else:
            self.dialog.mf_source.setCurrentText( str(mf_source) )
    ...
    def Project_Open(self):
        ...
        if ( project_zip != "" and project_zip != "." and self.project_zip != project_zip ):
            QMessageBox.information(QWidget(), i18n("Warnning"), i18n( str(project_zip) ))
    ...
    #endregion
    #region Mirror Fix #############################################################
    def MirrorFix_Source(self, mf_source):
        self.mf_source = mf_source
        # Save
        Krita.instance().writeSetting("Project Pages", "mf_source", str( self.mf_source ))
    def MirrorFix_Run(self):
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            # Read
            ki = Krita.instance()
            ad = ki.activeDocument()
            node = ad.activeNode()
            cond = self.dialog.mf_source.currentText()
            ...
            if (cond == "LEFT" or cond == "RIGHT"):
                ki.action('mirrorX-moveToCenter').trigger()
            if (cond == "UP" or cond == "DOWN"):
