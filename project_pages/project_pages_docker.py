...
class ProjectPages_Docker(DockWidget):
    ...
    def Connections(self):
        ...
        self.layout.page_list.clicked.connect(self.Text_Load)
    ...
    def Settings(self):
        ...
        # Rename Strings
        rename_strings = str( Krita.instance().readSetting("Project Pages", "rename_strings", "") )
        if rename_strings == "":
            Krita.instance().writeSetting("Project Pages", "rename_strings", "")
        else:
            self.Rename_LOAD( eval(rename_strings) )
    def Loader(self):
        # Start by showing Recent Projects (mode index = 0)
    ...
    def Mode_Index(self, index):
        ...
        if index == 0:
            ...
            # Module Containers
            self.layout.group_project.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            # Height
            self.layout.group_project.setMaximumHeight(qt_max)
            # Width
            self.layout.page_index.setMaximumWidth(0)

            self.Project_Thumbnail(self.recent_project)
        if index == 1:
            # Icon
            self.layout.mode.setIcon( self.qicon_page )
            # Module Containers
            self.layout.group_page.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            # Height
            self.layout.group_page.setMaximumHeight(qt_max)
            # Width
            self.layout.page_index.setMaximumWidth(120)
    ...
    def Display_Shrink(self):
        # Boards
        self.layout.group_project.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        self.layout.group_page.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        # Height
        self.layout.group_project.setMinimumHeight(0)
        self.layout.group_page.setMinimumHeight(0)
        self.layout.group_project.setMaximumHeight(qt_max)
        self.layout.group_page.setMaximumHeight(qt_max)
    ...
    def Menu_ProjectContext(self, event):
        ...
        # Selected Items
        project_item = self.layout.project_list.selectedItems()
        project_len = len(project_item)
        for i in range(0, project_len):
            project_item[i] = project_item[i].text()
        ...
        if project_len > 0:
            if action == cmenu_open:
                self.Project_Recent(project_item[0])
            if action == cmenu_clear:
                self.Project_Clear(project_item[0])
        if project_active == False:
            if action == cmenu_new:
                self.Project_New()
            elif action == cmenu_file:
                self.Project_File()
        else:
    ...
    def Menu_Mode_Press(self, event):
        ...
        cmenu = QMenu(self)
        # Actions
        cmenu_project = cmenu.addAction("Project")
        cmenu_page = cmenu.addAction("Page")
        # Icons
        cmenu_project.setIcon( self.qicon_project ) # all-layers current-layer duplicatelayer
        cmenu_page.setIcon( self.qicon_page ) #geometry palette-edit portrait select-all tool_rect_selection
        ...
        action = cmenu.exec_( position )
        # Triggers
        if action == cmenu_project:
            self.Mode_Index(0)
        elif action == cmenu_page:
            self.Mode_Index(1)
    ...
    def Project_Open(self):
        ...
        self.Project_Recent(project_item)
    def Project_Recent(self, project_item):
        for i in range(0, len(self.recent_project)):
            ...
            if name == project_item:
                ...
                self.Control_Load()
                break
    def Project_Clear(self, project_item):
        for i in range(0, len(self.recent_project)):
            name = os.path.basename( self.recent_project[i] ).replace(".project_pages.zip", "")
            if name == project_item:
                path = self.recent_project[i]
    ...
    def ZIP_New(self, path, name):
        if self.project_active == False:
            ...
            if exists == False:
                ...
                # Update
                self.layout.active_project.setText(os.path.basename(self.project_zip))
        ...
        if self.project_active == False:
            ...
            if exists == False:
                ...
                if valid == True:
                    ...
                    # Display
                    self.layout.active_project.setText(basename)
                    ...
                    QMessageBox.information(QWidget(), i18n("Warnning"), i18n("PP : Invalid File"))
            else:
                self.File_Conflict( project_directory )
        else:
            pass
        ...
        if self.project_active == True:
            ...
            if boolean == True:
                ...
                if closed == True:
                    ...
                    QMessageBox.information(QWidget(), i18n("Warnning"), i18n("PP : Close all Project Pages from canvas to run"))
    ...
    def File_Conflict(self, path):
        QMessageBox.information(QWidget(), i18n("Warnning"), i18n("PP : Name Conflict"))
        ...
        if (self.canvas() is not None) and (self.canvas().view() is not None):
            ...
            if (node_name != "" and node_name != "SUCCESS"):
                ...
                if node_i != None: # Avoids instant Krita Crash
                    ...
                    QMessageBox.information(QWidget(), i18n("Warnning"), i18n("PP : Conflict Solved"))
                    self.Layer_Rename()
...
"""
To do:
- Page animation tag (thumbnail, note, control)
- Drag and Drop to open ZIPs
